...
class MovieViewModel : ViewModel(), GenreInteractionListener, ResultInteractionListener {
    ...
    private val _genres = Repository.genres(true).asLiveData()
    ...
    private fun getMovie() {
        viewModelScope.launch {
            Repository.getGenreMovieOrTv(_requiredGenre.id, true).collect {
    ...
    override fun onClickItem(movieAndTvDetails: MovieAndTvDetails) {
        viewModelScope.launch {
            movieAndTvDetails.id?.let {
                Repository.movieDetails(it).collect { state ->
                    _selectedMovieEvent.postValue(Event(state))
                }
            }
        }
